		     ...
		     struct fc_log *l)
{
	...
	case Opt_ip:
		if (opt->flags & CEPH_OPT_MYIP) {
			error_plog(&log, "'ip' or 'server_ip' param is already specified\n");
			return -EINVAL;
		}
	...
	case Opt_server_ip:
		...
		err = ceph_parse_ips(param->string,
				     ...
				     1, NULL, true);
		if (err) {
			error_plog(&log, "Failed to parse server_ip: %d", err);
			return err;
		}
		opt->flags |= CEPH_OPT_MYIP;
		break;
			      ...
			      bool show_all)
{
	...
	if (show_all && (opt->flags & CEPH_OPT_NOOP_WRITE))
		seq_puts(m, "noop_write,");
