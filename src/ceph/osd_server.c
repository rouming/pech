...
DEFINE_RB_FUNCS2(osds_con, struct ceph_osds_con, con.peer_addr,
...
create_osd_repop(struct ceph_osds_msg *m, u32 epoch, int acktype)
{
	...
	ceph_msg_get(&m->msg);
	osds_rep->orig_msg = m;
	osds_rep->tid = pg->next_txn_id;
	RB_CLEAR_NODE(&osds_rep->node);

	msg->free_msg = free_repop_msg;
	msg->hdr.tid = cpu_to_le64(osds_rep->tid);
	pg->next_txn_id++;
					   ...
					   struct ceph_osds_msg *m)
{
	...
	for (i = 0, nr = 0; i < acting->size; i++) {
		...
		rep = create_osd_repop(m, osdc->osdmap->epoch,
			  /* TODO: Not actually clear to me when to set those */
			  CEPH_OSD_FLAG_ACK | CEPH_OSD_FLAG_ONDISK);
				...
				struct ceph_osds_msg *m)
{
	...
	ack_awaited = i_am_primary(osds, acting);
	if (ack_awaited) {
		m->nr_acks = 1;
		ceph_msg_get(&m->msg);
