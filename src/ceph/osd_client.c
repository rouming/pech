...
again:
	...
	if (osdc->abort_err) {
		...
		if (ceph_test_opt(osdc->client->options, ABORT_ON_FULL)) {
			err = -ENOSPC;
		} else {
	...
	if (opcode == CEPH_WATCH_EVENT_DISCONNECT) {
		...
		if (lreq->notify_id && lreq->notify_id != notify_id) {
			...
			if (data) {
				...
				/*
				 * Data pages are freed or ownership is taken,
				 * thus zero out and don't let double free
				 * happen on the following msg put path.
				 */
